...
async fn open_with_replica_write_then_read_round_trips() {
    ...
    s.put_link(quark::tenant::DEFAULT_TENANT, 101, &rec).await.unwrap();
    let got = s.get_link(quark::tenant::DEFAULT_TENANT, 101).await.unwrap().unwrap();
...
async fn open_and_open_with_replica_both_wire_working_pools() {
    ...
    single
        .put_link(quark::tenant::DEFAULT_TENANT, 201, &plain_rec("https://single.example"))
        .await
        .unwrap();
    assert_eq!(
        single.get_link(quark::tenant::DEFAULT_TENANT, 201).await.unwrap().unwrap().url,
    ...
    split
        .put_link(quark::tenant::DEFAULT_TENANT, 202, &plain_rec("https://split.example"))
        .await
        .unwrap();
    assert_eq!(
        split.get_link(quark::tenant::DEFAULT_TENANT, 202).await.unwrap().unwrap().url,
        "https://split.example"
    );
}
...
async fn put_get_link_pg() {
    ...
    s.put_link(quark::tenant::DEFAULT_TENANT, 7, &rec).await.unwrap();
    let got = s.get_link(quark::tenant::DEFAULT_TENANT, 7).await.unwrap().unwrap();
    ...
    assert!(s.get_link(quark::tenant::DEFAULT_TENANT, 999).await.unwrap().is_none());
}
...
async fn rules_round_trip_pg() {
    ...
    s.put_link(quark::tenant::DEFAULT_TENANT, 42, &rec).await.unwrap();
    let got = s.get_link(quark::tenant::DEFAULT_TENANT, 42).await.unwrap().unwrap();
...
async fn link_without_rules_round_trips_to_empty_vec_pg() {
    ...
    s.put_link(quark::tenant::DEFAULT_TENANT, 43, &rec).await.unwrap();
    let got = s.get_link(quark::tenant::DEFAULT_TENANT, 43).await.unwrap().unwrap();
...
async fn alias_is_atomic_no_orphan_pg() {
    ...
    assert!(s.put_alias_and_link(quark::tenant::DEFAULT_TENANT, "promo", 5, &rec).await.unwrap());
    assert!(!s.put_alias_and_link(quark::tenant::DEFAULT_TENANT, "promo", 9, &rec).await.unwrap());
    assert_eq!(s.get_alias(quark::tenant::DEFAULT_TENANT, "promo").await.unwrap(), Some(5));
    assert!(s.get_link(quark::tenant::DEFAULT_TENANT, 9).await.unwrap().is_none());
}
...
async fn tags_round_trip_filter_and_distinct_pg() {
    ...
    s.put_link(quark::tenant::DEFAULT_TENANT, 1, &rec("https://a.com", &["rust", "web"]))
        .await
        .unwrap();
    s.put_link(quark::tenant::DEFAULT_TENANT, 2, &rec("https://b.com", &["web"]))
        .await
        .unwrap();
    s.put_link(quark::tenant::DEFAULT_TENANT, 3, &rec("https://c.com", &[])).await.unwrap();

    let got = s.get_link(quark::tenant::DEFAULT_TENANT, 1).await.unwrap().unwrap();
    assert_eq!(got.tags, vec!["rust".to_string(), "web".to_string()]);

    let filtered = s.list_links(quark::tenant::DEFAULT_TENANT, None, 50, Some("rust"), None).await.unwrap();
    assert_eq!(
        filtered.iter().map(|(id, _)| *id).collect::<Vec<_>>(),
        vec![1]
    );

    let both_web = s.list_links(quark::tenant::DEFAULT_TENANT, None, 50, Some("web"), None).await.unwrap();
...
async fn folder_round_trip_filter_and_list_pg() {
    ...
    s.put_link(quark::tenant::DEFAULT_TENANT, 1, &rec("https://a.com", Some("Marketing")))
        .await
        .unwrap();
    s.put_link(quark::tenant::DEFAULT_TENANT, 2, &rec("https://b.com", Some("Marketing")))
        .await
        .unwrap();
    s.put_link(quark::tenant::DEFAULT_TENANT, 3, &rec("https://c.com", Some("Docs")))
        .await
        .unwrap();
    s.put_link(quark::tenant::DEFAULT_TENANT, 4, &rec("https://d.com", None)).await.unwrap();

    let got = s.get_link(quark::tenant::DEFAULT_TENANT, 1).await.unwrap().unwrap();
    assert_eq!(got.folder.as_deref(), Some("Marketing"));

    let filtered = s
        .list_links(quark::tenant::DEFAULT_TENANT, None, 50, None, Some("marketing"))
        .await
        .unwrap();
...
async fn visits_round_trip_pg() {
    ...
    s.put_link(quark::tenant::DEFAULT_TENANT, 11, &rec).await.unwrap();
    assert_eq!(s.visits(quark::tenant::DEFAULT_TENANT, 11).await.unwrap(), 0);
    assert_eq!(s.get_link(quark::tenant::DEFAULT_TENANT, 11).await.unwrap().unwrap().max_visits, Some(5));
}
...
async fn bump_visits_is_atomic_and_increments_pg() {
    ...
    s.put_link(quark::tenant::DEFAULT_TENANT, 12, &rec).await.unwrap();
    assert_eq!(s.bump_visits(quark::tenant::DEFAULT_TENANT, 12).await.unwrap(), 1);
    assert_eq!(s.bump_visits(quark::tenant::DEFAULT_TENANT, 12).await.unwrap(), 2);
    assert_eq!(s.visits(quark::tenant::DEFAULT_TENANT, 12).await.unwrap(), 2);
    ...
    for _ in 0..10 {
        ...
        handles.push(tokio::spawn(
            async move { s2.bump_visits(quark::tenant::DEFAULT_TENANT, 12).await.unwrap() },
        ));
    }
    ...
    assert_eq!(s.visits(quark::tenant::DEFAULT_TENANT, 12).await.unwrap(), 12);
}
...
async fn variants_round_trip_pg() {
    ...
    s.put_link(quark::tenant::DEFAULT_TENANT, 11, &rec).await.unwrap();
    let got = s.get_link(quark::tenant::DEFAULT_TENANT, 11).await.unwrap().unwrap();
    ...
    s.put_link(quark::tenant::DEFAULT_TENANT, 12, &plain).await.unwrap();
    assert!(s.get_link(quark::tenant::DEFAULT_TENANT, 12).await.unwrap().unwrap().variants.is_empty());
}
...
async fn wellknown_round_trip_pg() {
    ...
    assert_eq!(s.get_wellknown(quark::tenant::DEFAULT_TENANT, "assetlinks.json").await.unwrap(), None);
    let body = r#"{"relation":["delegate_permission/common.handle_all_urls"]}"#;
    s.put_wellknown(quark::tenant::DEFAULT_TENANT, "assetlinks.json", body).await.unwrap();
    assert_eq!(
        s.get_wellknown(quark::tenant::DEFAULT_TENANT, "assetlinks.json").await.unwrap(),
        Some(body.to_string())
    );
    s.delete_wellknown(quark::tenant::DEFAULT_TENANT, "assetlinks.json").await.unwrap();
    assert_eq!(s.get_wellknown(quark::tenant::DEFAULT_TENANT, "assetlinks.json").await.unwrap(), None);
    s.delete_wellknown(quark::tenant::DEFAULT_TENANT, "assetlinks.json").await.unwrap();
}
...
async fn put_link_tx_writes_link_and_deliveries_atomically() {
    ...
    s.put_link_tx(quark::tenant::DEFAULT_TENANT, 42, &rec, &[outbox_row(&key, sub.id, quark::now())])
        .await
        .unwrap();

    assert_eq!(
        s.get_link(quark::tenant::DEFAULT_TENANT, 42).await.unwrap().unwrap().url,
        "https://example.com"
    );
...
async fn put_link_tx_with_no_deliveries_upserts_link() {
    ...
    s.put_link_tx(quark::tenant::DEFAULT_TENANT, 43, &rec, &[]).await.unwrap();
    assert_eq!(
        s.get_link(quark::tenant::DEFAULT_TENANT, 43).await.unwrap().unwrap().url,
        "https://only-link.com"
    );
...
async fn put_alias_and_link_tx_writes_all_atomically() {
    ...
    let claimed = s
        .put_alias_and_link_tx(quark::tenant::DEFAULT_TENANT, "promo", 5, &rec, &[outbox_row(&key, sub.id, quark::now())])
        .await
        .unwrap();
    assert!(claimed);
    assert_eq!(s.get_alias(quark::tenant::DEFAULT_TENANT, "promo").await.unwrap(), Some(5));
    assert_eq!(
        s.get_link(quark::tenant::DEFAULT_TENANT, 5).await.unwrap().unwrap().url,
        "https://aliased.com"
    );
    assert_eq!(count_deliveries(&pool, &key).await, 1);
}
...
async fn put_alias_and_link_tx_conflict_rolls_back_link_and_deliveries() {
    ...
    assert!(s
        .put_alias_and_link_tx(quark::tenant::DEFAULT_TENANT, "promo", 5, &plain_rec("https://first.com"), &[])
        .await
        .unwrap());

    let key = format!("evt_test.{}", sub.id);
    let claimed = s
        .put_alias_and_link_tx(quark::tenant::DEFAULT_TENANT, 
            "promo",
            9,
...
async fn delete_link_tx_deletes_link_and_enqueues_deliveries() {
    ...
    let sub = add_sub(&s, "https://e.com/hook").await;
    s.put_link(quark::tenant::DEFAULT_TENANT, 77, &plain_rec("https://doomed.com"))
        .await
        .unwrap();
    let key = format!("evt_test.{}", sub.id);
    s.delete_link_tx(quark::tenant::DEFAULT_TENANT, 77, &[outbox_row(&key, sub.id, quark::now())])
        .await
        .unwrap();
    assert!(s.get_link(quark::tenant::DEFAULT_TENANT, 77).await.unwrap().is_none());
    assert_eq!(count_deliveries(&pool, &key).await, 1);
}
...
async fn put_link_tx_on_conflict_upserts_link_and_keeps_one_delivery() {
    ...
    let key = format!("evt_test.{}", sub.id);
    let at = quark::now();
    s.put_link_tx(quark::tenant::DEFAULT_TENANT, 
        50,
    ...
    .await
    .unwrap();
    assert_eq!(s.get_link(quark::tenant::DEFAULT_TENANT, 50).await.unwrap().unwrap().url, "https://v2.com");
    assert_eq!(
        count_deliveries(&pool, &key).await,
        1,
...
async fn link_health_round_trip_pg() {
    ...
    assert!(s.list_link_health(quark::tenant::DEFAULT_TENANT).await.unwrap().is_empty());

    s.put_link_health(quark::tenant::DEFAULT_TENANT, 
        1,
    ...
    let map: std::collections::HashMap<u64, quark::store::LinkHealth> =
        s.list_link_health(quark::tenant::DEFAULT_TENANT).await.unwrap().into_iter().collect();
    assert_eq!(map.len(), 2);
    assert_eq!(
        map[&1],
        quark::store::LinkHealth {
...
async fn list_broken_link_ids_pg() {
    ...
    .await
    .unwrap();
    assert_eq!(s.list_broken_link_ids(quark::tenant::DEFAULT_TENANT).await.unwrap(), vec![1, 2]);
}
...
async fn sheets_connection_round_trips_pg() {
    let Some(s) = fresh().await else { return };
    assert!(s.get_sheets_connection(quark::tenant::DEFAULT_TENANT).await.unwrap().is_none());
    ...
    s.put_sheets_connection(quark::tenant::DEFAULT_TENANT, &c).await.unwrap();
    let got = s.get_sheets_connection(quark::tenant::DEFAULT_TENANT).await.unwrap().unwrap();
    ...
    s.put_sheets_connection(quark::tenant::DEFAULT_TENANT, &c2).await.unwrap();
    let got = s.get_sheets_connection(quark::tenant::DEFAULT_TENANT).await.unwrap().unwrap();
    assert_eq!(got.email, "other@x.com");
    s.delete_sheets_connection(quark::tenant::DEFAULT_TENANT).await.unwrap();
    assert!(s.get_sheets_connection(quark::tenant::DEFAULT_TENANT).await.unwrap().is_none());
}
...
async fn session_round_trip_and_gc_pg() {
    ...
    s.put_session(quark::tenant::DEFAULT_TENANT, &quark::auth::Session {
        token_hash: "old".into(),
        expires: 5,
        ..sess.clone()
    })
