...
describe("Links", () => {
  ...
  it("renders the loaded links", async () => {
    vi.spyOn(globalThis, "fetch").mockResolvedValue(new Response(JSON.stringify({
      links: [{ id: 1, code: "6lB362J", url: "https://example.com/a", alias: "promo", expiry: null, created: 1700000000, rules: [] }],
      next_after: null,
    }), { status: 200 }));
  ...
  it("searches on the server when the backend supports it (?q= in the querystring)", async () => {
    const base = {
      links: [
        { id: 1, code: "AAA0000", url: "https://cat.com", expiry: null, created: 1, rules: [] },
        { id: 2, code: "BBB1111", url: "https://dog.com", expiry: null, created: 2, rules: [] },
      ],
      next_after: null,
    };
    mockFetchByUrl((url) =>
  ...
  it("falls back to client-side filtering when the search returns 501", async () => {
    const base = {
      links: [
        { id: 1, code: "AAA0000", url: "https://github.com/x", expiry: null, created: 1, rules: [] },
        { id: 2, code: "BBB1111", url: "https://example.com", expiry: null, created: 2, rules: [] },
      ],
      next_after: null,
    };
  ...
  it("search empty state shows the message with the term", async () => {
    const base = { links: [{ id: 1, code: "AAA0000", url: "https://cat.com", expiry: null, created: 1, rules: [] }], next_after: null };
    mockFetchByUrl((url) =>
  ...
  it("filters by tag via the API (?tag= in the querystring)", async () => {
    const base = {
      links: [
        { id: 1, code: "AAA0000", url: "https://cat.com", expiry: null, created: 1, tags: ["pets"], rules: [] },
        { id: 2, code: "BBB1111", url: "https://dog.com", expiry: null, created: 2, tags: ["pets", "promo"], rules: [] },
      ],
      next_after: null,
    };
    mockFetchByUrl((url) => {
