...
| `QUARK_CORS_ORIGINS` | unset (same-origin only) | Comma-separated list of origins allowed to call the API, for the separately hosted web panel. Empty means no CORS layer. Credentialed CORS is enabled (for the OIDC session cookie) when set. |
...
| `QUARK_SLACK_CLIENT_ID` (+ `QUARK_SLACK_CLIENT_SECRET`/`_REDIRECT_URL`) | unset (connector off) | Enables the Slack "Add to Slack" connector: an OAuth v2 install that returns an incoming webhook URL, stored as a `kind: slack` webhook subscription. Off unless all three are set. The only scope requested is `incoming-webhook`. See [WEBHOOKS](WEBHOOKS.md). |
| `QUARK_LOG_FORMAT` | unset (human-readable) | Set to `json` for one JSON object per log event, which is what a log pipeline wants. Anything else keeps the human-readable format. The log level is set by `RUST_LOG`, not by a `QUARK_` variable, because that is what `tracing-subscriber` reads and it also reaches the events emitted by tokio, sqlx and reqwest. The per-request access log lives at debug level: `RUST_LOG=tower_http=debug`. At the default `info` level the redirect hot path pays only a level check. |
...
| Variable | Default | Purpose |
|---|---|---|
| `QUARK_MULTI_TENANT` | unset / `0` (off) | Any value other than `0` turns on multi-tenant mode: FORCE row-level security, per-tenant transactions, and `Host`-based tenant resolution on the redirect path. Needs the Postgres store. Off means single-tenant, the OSS default. **Enterprise only.** Ignored by a Community build; see [LICENSING.md](LICENSING.md). |
| `QUARK_LICENSE_KEY` | unset | Enterprise subscription key for a self-hosted Enterprise build. Read once at boot, verified offline, never sent anywhere. Unset on an Enterprise build means `unlicensed`: reads keep working and Enterprise creation is refused, and the redirect path is never affected. **Enterprise only.** Ignored by a Community build; see [LICENSING.md](LICENSING.md). |
| `QUARK_TENANT_DOMAIN_SUFFIX` | unset (feature off) | Base suffix for the auto per-tenant subdomain, e.g. `quarkus.com.br` gives tenant `acme` the host `acme.quarkus.com.br`. Unset disables the whole subdomain-auto feature (no seed on tenant create, no boot backfill, `/admin/me` reports `null`). Cloud-only. **Enterprise only.** Ignored by a Community build; see [LICENSING.md](LICENSING.md). |
...
| `QUARK_KEYCLOAK_BASE_URL` (+ the `QUARK_KEYCLOAK_*` family) | unset (provisioning off) | Enables per-tenant Keycloak realm provisioning: the admin client (`_ADMIN_CLIENT_ID`/`_ADMIN_CLIENT_SECRET`), realm SMTP (`_SMTP_HOST`/`_PORT`/`_USER`/`_PASSWORD`/`_FROM`/`_STARTTLS`), `_LOGIN_THEME`, and `_PANEL_URL`. `_PANEL_URL` is the public base URL of the admin panel (e.g. `https://app.example.com`): when set, it is registered as a valid redirect URI (`{panel}/*`) and set-password emails return the user to `{panel}/login` instead of Keycloak's account console. Cloud-only; the full list and setup are in [RUNBOOK-keycloak-p2e](RUNBOOK-keycloak-p2e.md). **Enterprise only.** Ignored by a Community build; see [LICENSING.md](LICENSING.md). |
