...
senha de link, import, QR, o painel inteiro de um workspace, e o login OIDC
com um IdP único (que já está publicado; retirá-lo agora seria regressão).

**Vai para `src/ee/`.**

| Hoje | Linhas | Por que é EE |
|---|---|---|
| `src/api/tenants.rs` | 505 | criar, listar e excluir workspaces alheios |
| `src/api/invites.rs` mais `src/invite.rs` | 496 | convidar terceiros para uma conta |
| `src/api/sso_domains.rs` mais `src/sso.rs` | 339 | home-realm discovery por domínio de e-mail |
| `src/keycloak/` | 1149 | provisionar um realm de identidade por cliente |
| futuro LUC-41 | - | billing e gateway de pagamento |
| futuro LUC-64 | - | planos e limites por plano |
| futuro LUC-58 | - | white-label do painel |

**Vai para `web/src/ee/`.** `routes/Members.tsx`, `routes/SsoDomains.tsx`,
`routes/OidcProvider.tsx`, `routes/AcceptInvite.tsx`,
